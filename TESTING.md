...
| Delete Product(admin)      | Yes      | Navigated to Product, clicked 'delete'. | The admin can delete products that are in the databank or created by the admin from the backend. If a new product is added from the frontend, trying to delete this product leads to an error. |fail | fix the post and get for this process so that the product is created and stored in the databank and it´s image sent to AWS for retrieval.|
